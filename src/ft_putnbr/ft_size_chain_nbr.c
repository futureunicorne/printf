...
/*   Updated: 2017/02/03 15:11:13 by hel-hadi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_size_chain_nbr_bis(t_siz *siz, char *arg, int t_arg, int diff)
{
...
int	ft_size_chain_nbr_bis1(t_siz *siz, char *arg, int t_arg, int diff)
{
	siz->ecart =  siz->nbr_prec - t_arg;
	if (siz->nbr && siz->nbr_prec)
	{
...
void	ft_size_chain_nbr_bis2(t_siz *siz, char *s, ssize_t arg)
{
	if (ft_check_zero(s) == 1 && (siz->ecart <= 0 || !siz->nbr_prec))
		siz->d = '0';
...
int	ft_size_chain_nbr(char *s, int t_arg, int diff, ssize_t arg)
{
	...
	&& (t_arg > siz.nbr || ft_check_zero(s))) || (siz.nbr_prec > t_arg && siz.nbr_prec > siz.nbr)))
	{
		ft_putchar('+');
		siz.val++;
		siz.flag = 1;
	...
	ft_size_chain_nbr_bis2(&siz, s, arg);
	if (!siz.flag && ft_check_more(s) && ft_putplus_nbr_bis(s, arg))
	{
		ft_putchar('+');
		siz.val++;
