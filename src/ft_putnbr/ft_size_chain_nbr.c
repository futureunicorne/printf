...
/*   Updated: 2017/02/07 13:19:56 by hel-hadi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_size_chain_nbr_bis(t_siz *siz, char *s, int t_arg, ssize_t arg)
{
	...
	&& ((siz->nbr <= t_arg || siz->nbr_prec <= t_arg) || (ft_check_less(s)
	&& (siz->nbr > t_arg || siz->nbr_prec > t_arg))))
	{
		ft_putchar('+');
		siz->val++;
		siz->flag = 1;
	...
	if (ft_check_space(s) && (!ft_check_more(s)) && ft_putplus_nbr_bis(s, arg)
	&& (siz->nbr <= t_arg || arg == 0))
	{
		ft_putchar(' ');
		siz->val++;
